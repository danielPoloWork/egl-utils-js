...
export const TRANSFER_ROUTES = {
  ...
  artifact: {
    ...
    budget: 51680,
    why:
      'Re-pinned for 21.5 (+827 B for the F124-F125 tracker) — and re-pinned rather than left alone, because it still PASSED at 175 B under the old budget, which is not a margin to ship on. Before that, re-pinned for 21.4 (+1396 B for the F122-F123 submit lifecycle), holding the ADR-0059 rule. Before that, re-pinned for 21.3 (+1281 B for the F120-F121 renderer and the F120 class map), holding the same. Before that, re-pinned for 21.2 (+1609 B for the validation engine). Before that, re-pinned for 21.1 (+1178 B for the F112-F115 form engine and the twelfth-entry chunk re-split). Before that, re-pinned for 20.6 (+160 B for the F111 reduced-motion helper), holding the ADR-0059 rule: this is the file served as-is, which is what a CDN sends, while the size-limit row for the same path reports a smaller number because it re-bundles through esbuild first. Two honest measurements of two different things, kept separate on purpose (ADR-0061). ' +
