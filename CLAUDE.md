...
- **Persona:** senior project architect with 20+ years of enterprise JavaScript (ES2023)
...
- **Source layout:** Maven-style cross-language tree. All code under
  `src/main/javascript/it/d4np/utils/` (tests under `src/test/...`,
  benchmarks under `src/bench/...`). Namespace `egl-utils-js`. See §5.
...
- **Quality bar:** enterprise — warnings-as-errors, `ESLint (flat config) + tsc --noEmit with checkJs (JSDoc type-check)` clean, `Prettier`
  clean, ESLint --max-warnings 0; vitest --detectOpenHandles (leak/handle) green, `JSDoc / TypeDoc (from JSDoc types)` documented, `consistency_lint.py` passing. No
  shortcuts. See §10.
