...
Full conventions for branches, commits, and pull requests on `egl-utils-js`. The short
...
- `scope` is a short subsystem noun. Scopes for this repo: `api` `async` `events` `data` `validation` `web` `crypto` `storage` `sanitize` `errors` `diagnostics` `text` `net` `table` `logging` `dom` `bootstrap` `ui` `build` `tests` `docs` `ci` .
