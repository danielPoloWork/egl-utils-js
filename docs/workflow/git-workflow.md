...
- `scope` is a short subsystem noun. Scopes for this repo: `api` `async` `events` `data` `validation` `web` `crypto` `storage` `sanitize` `errors` `diagnostics` `text` `net` `table` `logging` `dom` `bootstrap` `build` `tests` `docs` `ci` .
