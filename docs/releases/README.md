...
|---------|------|------------|-------|
| v1.4.0 | 2026-08-27 | A form is a subsystem, and it now has one: values with the four coercions as contract, a validation engine that reads the platform rather than replacing it, findings rendered through an injected class map, a submit lifecycle whose double-submit guard is the promise itself, and dirty/touched tracking with a guard that exists only while there is something to guard | [v1.4.0.md](v1.4.0.md) |
| v1.3.0 | 2026-08-26 | The layer above a component — promise-based dialogs where a dismissal is an answer, a toast manager with a policy instead of an opinion, Bootstrap's own `data-bs-theme` with no flash and no second mechanism, breakpoints that mean what Bootstrap's source says, and the focus/announce primitives that had existed exactly once inside the overlay | [v1.3.0.md](v1.3.0.md) |
| v1.2.0 | 2026-08-24 | A table stops being a rendering of your data and starts being a component you can hand a user: remote data, URL state, row selection, CSV and clipboard export, and a header a user can freeze, resize and reorder — every pointer affordance with a keyboard path through the same control, and none of them reaching the derivation | [v1.2.0.md](v1.2.0.md) |
