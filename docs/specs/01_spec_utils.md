...
- NFR-01 Bundle budgets (min+gzip): root entry full import <= 6 kB; any single function <= 1 kB after shaking; /storage <= 2 kB — size-limit gate in CI
  - **Amended (roadmap 17.10, [ADR-0052](../adr/0052-withurlparams-moves-to-the-root.md)):** root entry full import <= **6.05 kB**. Re-exporting `withUrlParams` from the root (measured 6002 B, previously 5864 B) put the aggregate 50 B over the original clause — the first time the whole-entry ceiling has ever needed to grow rather than shrink, because it is the first time a function moved onto the root without ever being removed from its origin entry. The single-function ceiling (<= 1 kB) is untouched; this amends only the aggregate.
  - **Amended again (roadmap 17.12, [ADR-0053](../adr/0053-the-full-taxonomy-reaches-the-root.md)):** root entry full import <= **6.1 kB**. Re-exporting `DomContractError` and `PeerMissingError` from the root, to close the ADR-0003 gap the same item found, cost a further 70 B (measured 6072 B). Two amendments to the same clause inside one milestone is the visible cost of a root that keeps absorbing what earlier waves left off it — not a sign the gate is loose, since both changes were caught and sized before merging, not after.
  ...
  - **Resolved (roadmap 17.4, [ADR-0058](../adr/0058-the-per-function-budget-keeps-its-exceptions-named.md)):** the per-function clause **stays `<= 1 kB`, unqualified**, and a function that composes another public export and cannot meet it takes a **named, measured exception row**, documented by ADR in the PR that lands it — never silently, and never by deleting the row. The clause is not amended to exempt composing facades as a category: on this entry **27 of 28 functions comply**, and comfortably (the largest compliant is `retry` at 767 B, 25% under), so the pressure the clause applies is doing real work and a category exemption would remove it for one case. This is the rule spec 02 NFR-08 already states verbatim ("a composite that cannot meet 1 kB takes a named, measured exception row documented ADR-0015-style"); 17.4 writes it into the clause that lacked it. **Landed root exception: `httpClient` at 1.35 kB** (measured 1310 B — the composed `timeout` + `HttpError` of ADR-0007/ADR-0015).
...
- NFR-06 Zero runtime dependencies in the root entry (DOMPurify is peer + subpath only) — package.json audit in CI
  - **Amended (roadmap 17.11, [ADR-0051](../adr/0051-the-sanitizer-s-peer-range.md)):** the DOMPurify peer range is **`^3.4.13`**, not `^3` — the floor excludes every version known vulnerable when 1.0 froze it (GHSA-55q2-fjhq-7xh7, patched in 3.4.13). The range is a *compatibility* statement rather than a security mechanism: raising a peer floor is breaking, so advisories published after 1.0 are the consumer's to patch. The CI audit gate moves from `--audit-level high` to **`moderate`** in the same decision, because a DOMPurify advisory is the one finding in this tree that can require a change to `sanitizeHtml`'s own configuration.
- NFR-07 Portability: Node >= 18 LTS; last 2 evergreen Chromium/Firefox + Safari >= 15.4; one Web Crypto surface via conditional-exports shim; TypeScript >= 5.0 consumers resolve types through the exports map (spec §1.1)
  - **Amended (roadmap 17.14, [ADR-0054](../adr/0054-one-web-crypto-surface-without-the-conditions.md)):** "one Web Crypto surface via **conditional-exports shim**" becomes "one Web Crypto surface" — the clause's *promise* is unchanged and the mechanism is gone. The 17.2 floor made `globalThis.crypto` universal across the matrix (Node 19, Safari 11), so the `#webcrypto` `node`/`default` conditions selected between two files that had reduced to the same line. Collapsing them also removes the `dist/node` build pair and the exports map's `node` condition, which existed only to keep that shim's `node:crypto` import out of browser bundles. Supersedes [ADR-0008](../adr/0008-one-webcrypto-surface-conditional-exports.md).
...
**Scope note (roadmap 17.12, [ADR-0053](../adr/0053-the-full-taxonomy-reaches-the-root.md)):**
the enumeration below is spec 01's own frozen F1–F25 surface, unchanged since this spec was
written — it is not the *current* full public interface. Specs 02–04 each carry their own §5
listing what they add to the root and to each subpath entry (spec 02 §5, spec 03 §5, spec 04
§5); `VERSION` is a meta export outside the numbered items (ADR-0018). A single generated,
always-current reference is roadmap 17.3's deliverable.
...
- 'egl-utils-js/errors': EglError base + TimeoutError, RetryExhaustedError, AbortError (re-exported convention), HttpError, CloneError, StorageError, DurationParseError — all with stable .code (cross-realm-safe identity). Specs 03 and 04 each add one further class (`DomContractError`, `PeerMissingError`); **the whole taxonomy, present and future, is re-exported from the root** per ADR-0003 and [ADR-0053](../adr/0053-the-full-taxonomy-reaches-the-root.md), not only the 7 named here
