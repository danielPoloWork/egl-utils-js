...
- F50 loadingOverlay({onShow, onHide, minVisibleMs=400, focus?, signal?}) — a reference-counted visibility gate over an injected presentation pair, returning `{show, wrap, isShown, destroy}`. `show()` increments the count and returns an **idempotent** release function; the overlay hides when the count reaches zero, so nested or concurrent async operations cannot tear it down early. `wrap(promiseOrFn)` shows around an operation and always releases. The minimum-visible clock starts when `onShow` *settles*, not when `show()` is called, so a fast response cannot produce a flash, and a `hide` arriving mid-appearance is honoured once the appearance completes. With `focus.save` the active element is captured before showing, any focus inside the overlay is cleared before hiding, and focus is restored afterwards if the element is still in the document; `focus` is also the **only** option that needs a document, so a gate without it runs and is testable with no DOM. `isShown()` reports current state, `true` from the first `show()` including while the overlay is still appearing. `onShow`/`onHide` are called synchronously and their failures — a throw or a rejection — are **contained**: the gate returns to a consistent state and nothing is thrown into the caller, since a presentation that cannot render must not fail the operation it was decorating (ADR-0032, extending ADR-0027's containment rule). `destroy()` hides immediately, bypassing the floor, clears the timer, and cannot be resurrected by an `onShow` settling afterwards; an aborted `signal` destroys the gate too (NFR-15), and `show()` after `destroy()` throws `TypeError`
...
- NFR-12 Bundle budgets (min+gzip, size-limit gate in CI): **`/dom` ≤ 5 kB** — **amended
  ...
  indicative figure**). The **root entry is not touched by
