...
- F49 inlineAlert(container, {classes?, icons?, autoHideMs?, dismissible?, closeLabel?, signal?}) — a factory returning `{show(kind, message, options?), hide(), destroy()}` where `kind ∈ {'success','info','warning','danger'}`. Message text is rendered with `textContent`; rich content requires the `{html: true, sanitize}` pair, mirroring F47's contract. `classes` and `icons` are injected maps — defaults are neutral, framework-free class names, and **no icon set is assumed**; an icon may be a string, a node (cloned per render, so one map serves many instances), or a factory. Each instance owns its auto-hide timer and its close-button binding, so two alerts on one page can never cancel each other's timer or steal each other's container, and a re-show cancels the pending timer so the newer message gets its full time. `show` options are `{autoHideMs?, html?, sanitize?}`, where a per-show `0` or `Infinity` pins the alert open. The alert carries `role="alert"` for `warning`/`danger` and `role="status"` otherwise, and a dismissible instance renders a `type="button"` close control with a configurable accessible name (`closeLabel`, default `'Close'`). **Amended in 14.2** ([ADR-0038](../adr/0038-composites-compose-and-what-a-frozen-constant-costs.md)): an **empty close icon no longer hides the close control**. The hide-when-empty rule is right for the decorative icon span, whose empty box would still take margin, and wrong where the slot *is* the control — a design system that draws its close glyph entirely in CSS (Bootstrap's `.btn-close` background image) supplies an empty icon, and hiding the button for that leaves a dismissible alert nobody can dismiss. `dismissible: false` remains how a caller asks for no button; an empty glyph asks for no *glyph*. Nodes are built in the **container's own document** on first `show`, so an alert works inside an iframe or a server-side DOM implementation and an unused instance leaves the container untouched. `destroy()` removes the instance's node, its listeners and any pending timer; an aborted `signal` destroys it too (NFR-15), and `show()` after `destroy()` throws `TypeError` rather than silently doing nothing. **Amended in 17.9** ([ADR-0049](../adr/0049-commands-throw-queries-answer.md)): the instance is `{show, hide, isShown, element, destroy}` — `hide()` after `destroy()` now throws too (it was "harmless", which made the component inconsistent with its own `show()`), `isShown()` answers `false` rather than throwing, `element` is the container it was bound to, and the reported API name is **injectable** so `bsAlert` (spec 04 F64) names itself instead of leaking this engine's name into its diagnostics
- F50 loadingOverlay({onShow, onHide, minVisibleMs=400, focus?, signal?}) — a reference-counted visibility gate over an injected presentation pair, returning `{acquire, wrap, isShown, destroy}`. `acquire()` increments the count and returns an **idempotent** release function; the overlay hides when the count reaches zero, so nested or concurrent async operations cannot tear it down early. `wrap(promiseOrFn)` shows around an operation and always releases. The minimum-visible clock starts when `onShow` *settles*, not when `show()` is called, so a fast response cannot produce a flash, and a `hide` arriving mid-appearance is honoured once the appearance completes. With `focus.save` the active element is captured before showing, any focus inside the overlay is cleared before hiding, and focus is restored afterwards if the element is still in the document; `focus` is also the **only** option that needs a document, so a gate without it runs and is testable with no DOM. `isShown()` reports current state, `true` from the first `acquire()` including while the overlay is still appearing. `onShow`/`onHide` are called synchronously and their failures — a throw or a rejection — are **contained**: the gate returns to a consistent state and nothing is thrown into the caller, since a presentation that cannot render must not fail the operation it was decorating (ADR-0032, extending ADR-0027's containment rule). `destroy()` hides immediately, bypassing the floor, clears the timer, and cannot be resurrected by an `onShow` settling afterwards; an aborted `signal` destroys the gate too (NFR-15), and `acquire()` after `destroy()` throws `TypeError`. **Amended in 17.9** ([ADR-0049](../adr/0049-commands-throw-queries-answer.md)): `show` is renamed **`acquire`** — a visibility verb returns nothing in this library and this one hands back a lease — `wrap()` refuses a destroyed gate as well, and this is the one instance in the library with **no `element`**, deliberately: the gate owns *when* an overlay is visible and nothing about *what* is
- F51 bindTableControls(pipeline, bindings, {signal?, debounceMs=200, root=document, formatStatus?}) — wires DOM controls to an F42 pipeline through its public commands only, and reflects state back on every `'change'`. `bindings`: `{filters?: {key: selector}, search?: selector, sortHeaders?: {root, selector}, pagination?: {prev?, next?, status?}, pageSize?: selector}`. Filter and search inputs are debounced (F7); sort headers use **one** delegated listener (F44) and receive `aria-sort`; pagination controls are enabled/disabled from the derived view. Returns an unbind function; teardown is structural — one internal `AbortController` detaches every listener, cancels every pending debounce, and unsubscribes from the pipeline, so no trailing callback can fire into a dead binding. **Row rendering stays the caller's**: the binding wires controls, never cells. **Amended in 13.2** on two points the pre-implementation clause left open: the pagination status text is written by an injected `formatStatus(view)` whose default is the language-neutral `'1 / 4'`, because a default reading "Page 1 of 4" would ship an English string into every consumer's UI and the wave's own rule is that policy is injected; and a selector matching **nothing** throws `DomContractError` naming every miss at once, since passing a selector asserts the control exists and a typo that silently disables a filter box is precisely the failure ADR-0028 fails fast to prevent — controls a table genuinely lacks are omitted from `bindings` instead. Reflection is deliberately one-way for the filter and search inputs: a control that writes back into the field its user is typing in fights its own user
...
- NFR-12 Bundle budgets (min+gzip, size-limit gate in CI): **`/dom` ≤ 5 kB** — **amended
  in 13.2 from 4 kB on measuring 4718 B once F51 pulled the F7 debounce onto the entry, by
  [ADR-0035](../adr/0035-the-controls-bridge-and-the-dom-budget.md)** — and the
  ...
  `bindTableControls` ≤ 1.75 kB — **pinned at 2.2 kB on measuring 2093 B in 13.2 by
  [ADR-0035](../adr/0035-the-controls-bridge-and-the-dom-budget.md), the F7 debounce and the
  F44 delegation it composes being the bulk of it** —
  ...
  amended to 1.58 kB in 17.9 on measuring 1533 B by
  [ADR-0049](../adr/0049-commands-throw-queries-answer.md) — `isShown()`, `element` and the
  injectable api name that stops `bsAlert` blaming this engine** — `loadingOverlay`
  ≤ 1.25 kB, **pinned at 1.03 kB on measuring 958 B in 12.2, within the indicative figure;
  row raised to 1.1 kB in 17.9 on measuring 1063 B, still inside the clause**). The **root entry is not touched by
