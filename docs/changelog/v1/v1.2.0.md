...
v1.0.0 froze a 113-export surface; this release adds ten exports and six `bsTable` options to
it, and changes none of the 113. Milestone 19 delivers spec 06 (F88–F100) in seven capability
items plus a gate fix and a defect fix, and every one of them attaches *beside* the F42
derivation rather than inside it — which is what NFR-25's additive-only clause looks like when
it is a hard requirement rather than an intention.

Two threads run through the wave. The first is that **data can now come from somewhere else**:
a server, a URL, a saved layout. The second is that **the user can now change what they see** —
select rows, export them, freeze the header, resize a column, reorder one — without any of it
reaching the pipeline that decides *which* rows they are.

Eight ADRs record the decisions:
[0062](../../adr/0062-a-sibling-not-a-wrapper.md),
[0063](../../adr/0063-the-url-is-the-state-and-the-page-goes-last.md),
[0064](../../adr/0064-the-gate-that-was-watching-nothing.md),
[0065](../../adr/0065-a-set-of-keys-and-the-page-it-can-see.md),
[0066](../../adr/0066-a-csv-is-not-an-inert-document.md),
[0067](../../adr/0067-five-declarations-and-no-scroll-listener.md),
[0068](../../adr/0068-a-colgroup-a-separator-and-a-ceiling-in-sight.md) and
[0069](../../adr/0069-an-order-is-a-permutation-and-the-ceiling-held.md).
...
- **The async contract is proved against races, not examples** (NFR-26). A property suite runs
  randomised interleavings of commands and responses — out-of-order arrival, abort-then-arrive,
  identical-query coalescing — and asserts that no interleaving produces a view whose rows came
  from a query other than the one that view describes. A race is not a thing example tests find;
  they find the races someone imagined.

- **Selection is O(selected), not O(source)** (NFR-27). Selecting all on a 10 000-row source and
  reading `getSelection()` with 10 000 keys held are each under 10 ms on the benchmark machine,
  and an unselected 100 000-row table pays nothing — a `Set` of keys, not a flag per row. F42's
  own NFR-13 derivation budget is unchanged and re-verified.

- **Every pointer feature is asserted in a real engine.** jsdom has no layout, so it owns the
  models and the arithmetic; Chromium, Firefox and WebKit own the geometry — that a sticky header
  stays at the container's top edge while row one scrolls above it, that a drag moves a column
  boundary and the width survives the pointer leaving the table, that a reorder drag swaps the
  columns under the cursor and the filter row travels with them, and that the resize grip and the
  move handle never share a pixel of a header.

- **The api-floor gate now sees what it was missing.** Roadmap 19.8 found `check-api-floor.mjs`
  stripping template literals whole — so `` `${location.pathname}` `` was invisible — and
  matching members only through a bare `.`, so optional chaining was invisible too. Both are
  fixed and the scanner has its own test suite; `globalThis.location?.protocol` in `storage.js`
  had never been seen by the gate that exists to see it
  ([ADR-0064](../../adr/0064-the-gate-that-was-watching-nothing.md)).

- **Seven platform-API amendments, each explicit** (NFR-28): `history.pushState`/`replaceState`/
  `state` and four `location` members for F93, the async Clipboard API for F97, and Pointer
  Events plus pointer capture and `getBoundingClientRect` for F99. Every one is inside the
  Safari 16.4 / Node 22 floor, and every one is declared rather than assumed.

## Notes

- **Additive-only, mechanically.** The public surface goes from 113 exports to **123**; not one
  of the 113 changes name, shape or meaning, no `EGL_*` code is renamed, and the `exports` map is
  untouched. One new code — `EGL_CLIPBOARD` — is an addition to the registry, which ADR-0003
  already classifies as minor.

- **Two budget ceilings are close, and that is recorded rather than discovered.** The browser
  artifact ends the wave at **38 990 B of NFR-22's 40 kB**, and ADR-0041's 25 kB `/bootstrap`
  entry clause has **≈ 594 B** left. M20 puts dialogs, a toast manager and a theme manager on
  that same entry, so the first item that grows it owes a decision with arithmetic behind it —
  amend the clause, or split the entry. Roadmap 19.9 is the concrete measure of how close that
  is: a one-cell defect fix cost 92 B.

- **Excel export stays out of core.** It needs a runtime dependency, and the zero-dependency rule
  is not negotiable for it; a caller callback is the extension point (spec 06 F96).

- **A column move is not announced.** A screen-reader user hears the header row's new order when
  they re-read it, not at the moment of the move — this library has no live region until M20.5
  builds one. F100 asks for operability, which is met; the announcement is a named gap.
