...
v1.0.0 froze a 113-export surface and v1.2.0 grew it to 123. This release adds **ten exports
and one new entry** — `egl-utils-js/ui` — and changes none of the 123. Milestone 20 delivers
spec 07 (F101–F111) in six capability items plus a verification-infrastructure fix, and the
whole wave is additive by construction: no existing export signature, option name, `EGL_*`
code or `exports`-map path moved (NFR-31).

The catalogue was already finished. What was missing is the code every application writes
*around* it: a dialog that answers, a notification stream with a policy, a preference that
survives a reload, and the two accessibility primitives that had existed exactly once each,
inside the F50 overlay where nothing could reach them.

Six ADRs record the decisions:
[0070](../../adr/0070-two-primitives-extracted-and-a-ceiling-recomputed.md),
[0071](../../adr/0071-a-manager-not-three-globals-and-a-dismissal-is-an-answer.md),
[0072](../../adr/0072-a-queue-a-rule-nobody-has-to-guess-and-one-toast-per-story.md),
[0073](../../adr/0073-bootstraps-own-attribute-and-a-snippet-that-cannot-drift.md),
[0074](../../adr/0074-bootstraps-own-mixins-five-queries-and-a-seam-written-once.md) and
[0075](../../adr/0075-one-query-point-and-a-seam-that-crossed-a-boundary.md). A seventh,
[0076](../../adr/0076-a-worker-count-decided-and-a-server-that-was-not-the-cause.md), belongs
to the wave but changes no shipped code — see *Verification* below.
...
- **`focusTrap`, `saveFocus` and `liveRegion` on `egl-utils-js/dom`** — the two things every
  ...
  **holds focus itself** rather than losing it to `<body>` — the case that otherwise turns a
  ...
  [ADR-0070](../../adr/0070-two-primitives-extracted-and-a-ceiling-recomputed.md)).
...
Not part of the published surface, and recorded because the wave is what found it: the
three-engine browser suite was failing 3–4 tests per full run, a different set each time.
20.7 measured the diagnosis it had been filed with — that the test static server was the
bottleneck — and disproved it: the cause was Playwright's default worker count, *half the
machine's cores*, a rule that scales the wrong way. Bounded to `min(4, cores/2)` the suite runs
160/160 and **faster** (1m40 against 2m26), one pinned timeout replaces six per-file copies,
and `retries` stays 0 in writing
