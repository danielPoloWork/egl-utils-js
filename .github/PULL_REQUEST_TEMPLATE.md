...
- [ ] Builds cleanly on the full CI matrix (Linux (Node.js 18, 20, 22))
- [ ] Unit tests pass; new/changed behavior covered (≥ 95% line)
- [ ] `Prettier` clean; `ESLint (flat config) + tsc --noEmit with checkJs (JSDoc type-check)` clean on the diff
- [ ] ESLint --max-warnings 0; vitest --detectOpenHandles (leak/handle) green (where applicable)
