...
// The Safari-15.4 fallback this block used to exercise is gone: the 1.x floor
// (Node >= 22, Safari >= 16.4 — ADR-0050) has `AbortSignal.timeout` natively
// everywhere, so `timeout` calls the static directly. What is still worth
// asserting is the property the fallback existed to preserve.
describe('timeout — the platform static, on the 1.x floor (NFR-07, ADR-0050)', () => {
  it('uses AbortSignal.timeout rather than a hand-rolled timer', async () => {
    const spy = vi.spyOn(AbortSignal, 'timeout');
    await timeout(async () => 'ok', 50);
    expect(spy).toHaveBeenCalledWith(50);
    spy.mockRestore();
  });

  it("aborts the operation's signal with the platform's own TimeoutError reason", async () => {
    // The reason matters downstream: `httpClient` composes `timeout`, and code
    // that inspects `signal.reason` must see what the platform would produce.
    /** @type {AbortSignal | undefined} */
    let observed;
